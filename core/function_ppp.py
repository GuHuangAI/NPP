...
# @torchsnooper.snoop()
# ml = MulLoss()
def train(config, epoch, num_epoch, lr, trainloader, optimizer, model, criterion_pose, criterion_par, writer_dict,
          device):
    # Training
    model.train()
...
def train_apex(config, epoch, num_epoch, lr, trainloader, optimizer, model, criterion_pose, criterion_par, writer_dict,
               device, amp):
    # Training
    model.train()
    batch_time = AverageMeter()
...
def train_with_alpha(config, train_loader, mini_loader, model, criterion_pose, criterion_par, optimizer, a_optimizer,
                     epoch,
...
def validate_sync(config, testloader, model, criterion_pose, criterion_par, writer_dict, device):
    rank = get_rank()
    world_size = get_world_size()
    #print(world_size)
    model.eval()
    ave_loss = AverageMeter()
    # acc = AverageMeter()
    acc = MulAverageMeter(15)
    confusion_matrix = np.zeros(
        (config.DATASET.NUM_CLASSES, config.DATASET.NUM_CLASSES))
    # im_list=[]
    #pred_list = np.zeros((len(im_list), 14, 3))
    #n = 0
    #pose_pred_path = config.POSE_PRED_PATH[:-4] + '{}.csv'.format(rank)
    #pose_gt_path = config.POSE_GT_PATH
    with torch.no_grad():
        for i_iter, batch in enumerate(testloader):
            ...
            pred_pose, pred_par = model(image)
            flip_pred_pose, flip_pred_par = model(flip_img)
            flipped_poseidx = [0, 1, 8, 9, 10, 11, 12, 13, 2, 3, 4, 5, 6, 7]
            ...
            for ji in range(0, 14):
                pred_pose[:, ji, :, :] = 0.5 * (pred_pose[:, ji, :, :].copy() + flip_pred_pose[:, flipped_poseidx[ji], :, :])
            acc1, avg_acc, cnt, _ = accuracy(pred_pose, label_pose[0].data.cpu().numpy())
            acc.update(acc1)
    pck_table_output_ppp_dataset(acc.val())
    pck_avg = torch.from_numpy(np.array(acc.val())).to(device)
    pck_avg = reduce_tensor(pck_avg)
    pck_avg = pck_avg.cpu().numpy() * 100 / get_world_size()
    print(pck_avg)
    ...
    #              'rightShoe']
    class_name = [ 'background', 'head', 'torso', 'upper-arm', 'lower-arm', 'upper-leg', 'lower-leg' ]
    if rank == 0:
        writer = writer_dict['writer']
        global_steps = writer_dict['valid_global_steps']
        writer.add_scalar('valid_loss', print_loss, global_steps)
        writer.add_scalar('valid_mIoU', mean_IoU, global_steps)
        writer.add_scalar('valid_acc', pck_avg[0], global_steps)
        writer_dict['valid_global_steps'] = global_steps + 1
    ...
    return print_loss, mean_IoU, IoU_array, pck_avg[0]

def pck_table_output_ppp_dataset(pck, method_name='Ours'):
    str_template = '{0:10} & {1:6} & {2:6} & {3:6} & {4:6} & {5:6} & {6:6} & {7:6} & {8:6} & {9:6}'
    head_str = str_template.format('PCKh@0.5', 'fore', 'neck.', 'sho.', 'elb.', 'wri.', 'hip', 'knee', 'ank.', 'Avg.')
    num_str = str_template.format(method_name, '%1.1f'%(pck[1]),
                                               '%1.1f'%(pck[2]),
                                               '%1.1f'%((pck[3] + pck[9]) / 2.0),
                                               '%1.1f'%((pck[4] + pck[10]) / 2.0),
                                               '%1.1f'%((pck[5] + pck[11])  / 2.0),
                                               '%1.1f'%((pck[6] + pck[12])  / 2.0),
                                               '%1.1f'%((pck[7] + pck[13])  / 2.0),
                                               '%1.1f'%((pck[8] + pck[14])  / 2.0),
                                               '%1.1f'%(pck[0]))
    print(head_str)
    print(num_str)
...
def validate_sync2(config, testloader, model, im_list, criterion_pose, criterion_par, writer_dict, device):
    ...
    return print_loss, mean_IoU, IoU_array, acc.val()[0]
