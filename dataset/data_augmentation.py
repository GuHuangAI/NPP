...
def augmentation_scale(im, scale_self, target_dist=1.171, scale_min=0.8, scale_max=1.5, istrain=True, crop_size=384.):
	#print "Data augmentation -- Scale"
	
	dice = random.random()
	scale_multiplier = (scale_max - scale_min) * dice + scale_min
	long_size = max(im.shape[0], im.shape[1])
	base_scale = crop_size /  long_size
...
def augmentation_scale2(im, scale_self, target_dist=1.171, scale_min=0.8, scale_max=1.5):
	#print "Data augmentation -- Scale"
...
# 	scale_abs = target_dist / scale_self
	long_size = max(im.shape[0], im.shape[1])
	scale_multiplier = 384. /  long_size
	if scale_self == 0:
		scale_self = 1
# 	scale_abs = target_dist / scale_self
...
def ori_joints_trans(joints, ori_size, crop_size):
    # i=0
    # ori_size=e['size']
    # crop_size=(256,256)
    for i in range(joints.shape[0]):
        joints[i,0] = joints[i,0]*crop_size[1]/ori_size[1]
        joints[i,1] = joints[i,1]*crop_size[0]/ori_size[0]
    return joints
