...
class BaseDataset(data.Dataset):
    ...
    def rand_crop(self, image, label1,label2):
        h, w = image.shape[:-1]
        image = self.pad_image(image, h, w, self.crop_size,
                                (0.0, 0.0, 0.0))
        label1 = self.pad_image(label1, h, w, self.crop_size,
                                (self.ignore_label,))
        l=[]
        for i in range(label2.shape[2]):
            temp = self.pad_image(label2[:,:,i], h, w, self.crop_size,
                                    (0,))
            l.append(temp)
        label2 = np.stack(l, axis=2)
            
            
        
        new_h, new_w = label1.shape
        x = random.randint(0, new_w - self.crop_size[1])
        y = random.randint(0, new_h - self.crop_size[0])
        image = image[y:y+self.crop_size[0], x:x+self.crop_size[1]]
        label1 = label1[y:y+self.crop_size[0], x:x+self.crop_size[1]]
        label2 = label2[y:y+self.crop_size[0], x:x+self.crop_size[1]]
    ...
    def image_resize(self, image, long_size, label1=None,label2=None):
        h, w = image.shape[:2]
        if h > w:
            new_h = long_size
        ...
        if label1 is not None:
            label1 = cv2.resize(label1, (new_w, new_h), 
                           interpolation = cv2.INTER_NEAREST)
            label2 = cv2.resize(label2, (new_w, new_h), 
                           interpolation = cv2.INTER_NEAREST)
        else:
            return image
    ...
    def multi_scale_aug(self, image, label1=None, label2=None,
            rand_scale=1, rand_crop=True):
        long_size = np.int(self.base_size * rand_scale + 0.5)
        if label1 is not None:
            image, label1, label2 = self.image_resize(image, long_size, label1,label2)
            if rand_crop:
                image, label1 ,label2 = self.rand_crop(image, label1,label2)
            return image, label1, label2
        else:
            image = self.image_resize(image, long_size)
            return image

    def gen_sample(self, image, label1, label2,
            multi_scale=True, is_flip=True, center_crop_test=False):
        if multi_scale:
            rand_scale = 0.5 + random.randint(0, self.scale_factor) / 10.0
            image, label1,label2 = self.multi_scale_aug(image, label1, label2,
