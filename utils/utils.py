...
def count_parameters_in_MB(model):
    out = 1024*1024
    return np.sum(np.prod(v.size()) for name, v in model.named_parameters() if "auxiliary" not in name)/out
...
def get_confusion_matrix(label, pred, size, num_class, ignore=-1):
    ...
    if isinstance(pred,list):
        output=pred[0].cpu().numpy().transpose(0, 2, 3, 1)
    else:
...
def pck_table_output_lip_dataset(pck, method_name='Ours'):
    ...
    num_str = str_template.format(method_name, '%1.1f'%((pck[1] + pck[2])  / 2.0),
                                               '%1.1f'%((pck[3] + pck[6]) / 2.0),
                                               '%1.1f'%((pck[4] + pck[7]) / 2.0),
                                               '%1.1f'%((pck[5] + pck[8]) / 2.0),
                                               '%1.1f'%((pck[9] + pck[12])  / 2.0),
                                               '%1.1f'%((pck[10] + pck[13])  / 2.0),
                                               '%1.1f'%((pck[11] + pck[14])  / 2.0),
                                               '%1.1f'%((pck[15] + pck[16])  / 2.0),
                                               '%1.1f'%(pck[0]))
    print(head_str)
    print(num_str)
